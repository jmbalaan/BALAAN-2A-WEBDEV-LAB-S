...
<body>
    ...
    <?php
        ...
        function computePizzaTotal($pizzaPrice, $selectedToppings, $toppingsMenu, $qty) {
            $toppingsTotal = 0;

            // Add up the price of each selected topping
            for ($i = 0; $i < count($selectedToppings); $i++) {
                $toppingName = $selectedToppings[$i];
                if (isset($toppingsMenu[$toppingName])) {
                    $toppingsTotal = $toppingsTotal + $toppingsMenu[$toppingName];
                }
            }

            // Multiply by quantity to get grand total
            $grandTotal = ($pizzaPrice + $toppingsTotal) * $qty;
            return $grandTotal;
        }
    ?>
    ...
    <div class="container">
        ...
        <div class="content">
                        ...
                        <div class="radio-group">
                            <?php
                                ...
                                foreach ($pizzaMenu as $name => $price) {
                                    ...
                                    echo '<input type="radio" name="pizza" id="pizza_' . $name . '" value="' . $name . '" required>';
                                    echo '<label for="pizza_' . $name . '">';
                                    echo '<span>' . $name . '</span>';
                        ...
                        <div class="checkbox-group">
                            <?php
                                ...
                                foreach ($toppingsMenu as $name => $price) {
                                    ...
                                    echo '<input type="checkbox" name="toppings[]" id="topping_' . $name . '" value="' . $name . '">';
                                    echo '<label for="topping_' . $name . '">';
                                    echo '<span>' . $name . '</span>';
                                    echo '<span class="price">+₱' . $price . '</span>';
                                    echo '</label>';
                                    echo '</div>';
            ...
            <div class="card">
                ...
                <?php
                    if (isset($_POST['order'])) {
                        ...
                        $grandTotal = computePizzaTotal($pizzaPrice, $selectedToppings, $toppingsMenu, $qty);

                        // TODO: 6. Display Results
                        echo '<div class="summary-item"><span>👤 Customer</span><span>' . $customer . '</span></div>';
                        echo '<div class="summary-item"><span>🍕 Pizza</span><span>' . $selectedPizza . ' — ₱' . $pizzaPrice . '</span></div>';
                        ...
                        if (count($selectedToppings) > 0) {
                            ...
                            for ($i = 0; $i < count($selectedToppings); $i++) {
                                ...
                                $toppingsList = $toppingsList . $selectedToppings[$i] . " (+₱" . $toppingsMenu[$selectedToppings[$i]] . ")";
                            }
                            echo '<div class="summary-item"><span>🧀 Toppings</span><span>' . $toppingsList . '</span></div>';
                        } else {
                            echo '<div class="summary-item"><span>🧀 Toppings</span><span>None</span></div>';
                        }

                        echo '<div class="total">Grand Total: ₱' . $grandTotal . '</div>';
            ...
            <div class="card full-width">
                ...
                <div class="menu-grid">
                    <?php
                        // Display all pizzas in the menu grid
                        foreach ($pizzaMenu as $name => $price) {
                            echo '<div class="menu-item">';
                            echo '<div class="menu-item-name">🍕 ' . $name . '</div>';
                            echo '<div class="price">₱' . $price . '</div>';
                            echo '</div>';
                        }
                        // Display all toppings in the menu grid
                        foreach ($toppingsMenu as $name => $price) {
                            echo '<div class="menu-item">';
                            echo '<div class="menu-item-name">🧀 ' . $name . '</div>';
                            echo '<div class="price">+₱' . $price . '</div>';
            ...
            <div class="card full-width order-counter">
                ...
                <?php
                    if (isset($_POST['order'])) {
                        ...
                        for ($i = 1; $i <= $qty; $i++) {
                            echo '<div class="order-item">Order #' . $i . ' — ' . $selectedPizza . ' Pizza for ' . $customer . '</div>';
                        }
                    }
                ?>
            </div>
